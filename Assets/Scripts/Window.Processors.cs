    ...
    public partial class Window {
        private static void ProcessReadyFrame(ReadyFrame readyFrame) {
            ...
            foreach (var c in lobbyChannels.Values) {
                if (c.lastMessageId != null) c.lastMessage = lastMessagesMapping[c.lastMessageId];

                if (OverrideNames.ContainsKey(c.id)) c.name = OverrideNames[c.id];
            }

            foreach (var pair in readyFrame.data.groupMap) Groups.putIfAbsent(pair.Key, () => pair.Value);
        ...
        private static void ProcessMessageCreateFrame(MessageCreateFrame messageCreateFrame) {
            ...
            if (frameMessage.type == "normal") Users[frameMessage.author.id] = frameMessage.author;
            ...
            if (_instance != null)
                using (_instance.window.getScope()) {
                    if (HomePage.currentState != null) selectedChannelId = HomePage.currentState.SelectedChannelId;
                }
...
/*                UnityMainThreadDispatcher.Instance().Enqueue(() => NotificationManager.instance.ShowNotification(
        ...
        private static void ProcessChannelCreateFrame(ChannelCreateFrame channelCreateFrame) {
            ...
            if (frameData.type == "lobby" &&
                ...
                if (!Members.ContainsKey(frameData.id)) Members[frameData.id] = new List<ChannelMember>();
